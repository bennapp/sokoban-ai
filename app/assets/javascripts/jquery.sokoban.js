...
(function($){
  ...
  function AiSokoban (level, moves) {
    this.level = $( level );
    this.original = this.level.text();
    var ok = this.processLevelData();
    if ( ok ) {
  ...
  AiSokoban.prototype = {
    ...
    processLevelData: function () {
      ...
      if ( men !== 1 || docks !== boxes || this.height < 3 || this.width < 3 ) {
        alert("this is an unplayable level");
        return false;
      }
      this.level
        .empty()
  ...
  $.fn.ai_sokoban = function () {
    return this.each(function(){
      ...
      if ( !ctl && !elm.children().length ) {
        ...
        $this = $(this);
        var moves = $("." + $this.attr('class') + '-moves').text().split(",");
        console.log(moves);

        elm.data( 'sokoban', new AiSokoban( this, moves ) );
      }
    });
  };
